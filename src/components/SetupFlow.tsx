import { Check, ChevronLeft, ChevronRight, CircleUserRound, CreditCard, Crosshair, ImagePlus, LoaderCircle, Mail, PawPrint, RefreshCcw, RotateCcw, ShieldCheck, SlidersHorizontal, Sparkles, Trash2 } from "lucide-react";
import { ChangeEvent, useEffect, useState } from "react";
import { listen } from "@tauri-apps/api/event";
import { isTauri } from "@tauri-apps/api/core";
import { getCurrentWindow } from "@tauri-apps/api/window";
import type { AppSnapshot, Personality, PetConfig, RigAnalysis, RigFamily } from "../types";
import { EMPTY_SNAPSHOT } from "../types";
import { activatePet, assetUrl, cancelGeneration, deletePet, discardPetCandidate, getSnapshot, selectPet, startGeneration, submitRigCorrections, useImageCandidate, useModelCandidate } from "../lib/native";
...
export function SetupFlow() {
  ...
  function placeRigPoint(position: [number, number, number]) {
    ...
    const landmarks = rigGuide.landmarks.map((landmark) => landmark.name === activeLandmark ? { ...landmark, position, confidence: 1 } : landmark);
  ...
  function createNewPet() {
    setConfig(newPetConfig()); setFile(undefined); setImageUrl(undefined); setError(undefined); setRigGuide(undefined); setActiveLandmark(undefined); setScreen("onboarding"); setStep(1);
  }
