...
export function SetupFlow() {
  ...
  async function saveRigGuide() {
    ...
    setBusy(true); setError(undefined);
    // Move to the normal progress screen immediately. The Rust command keeps
    // polling Blender, so leaving the user on a spinning submit button makes a
    // healthy long-running rig look frozen.
    setStep(2);
    try { await submitRigCorrections(rigGuide); setSnapshot(await getSnapshot()); }
    catch (e) { setError(String(e)); setSnapshot(await getSnapshot()); } finally { setBusy(false); }
  }

  async function retryBlenderRig() {
    setBusy(true); setError(undefined); setStep(2);
    try { await retryRigging(); setSnapshot(await getSnapshot()); }
    catch (e) { setError(String(e)); setSnapshot(await getSnapshot()); } finally { setBusy(false); }
  }

  async function retryAnimationFlow() {
    setBusy(true); setError(undefined); setStep(2);
    try { await retryAnimation(); setSnapshot(await getSnapshot()); }
    catch (e) { setError(String(e)); setSnapshot(await getSnapshot()); } finally { setBusy(false); }
  }
  ...
  const candidateModelPath = snapshot.generation.candidateModelPath ?? "";
  const canRetrySavedRig = snapshot.generation.rigAnalysis?.status === "corrected" && /(^|[\\/])base\.glb$/i.test(candidateModelPath);
  const canRetrySavedAnimation = snapshot.generation.rigAnalysis?.status === "corrected" && /(^|[\\/])rigged\.glb$/i.test(candidateModelPath);
  ...
  return <main className="setup-shell">
    ...
    <section className="setup-card">
      ...
      <div className="setup-content">
        ...
        {step === 2 && <div className="panel-body progress-panel"><div className="creation-orbit"><span className="orbit-ring" /><Sparkles size={34} /></div><p className="eyebrow">STEP 3 OF 5</p><h2>{snapshot.generation.stage === "completed" ? "Your 3D pet is ready" : snapshot.generation.stage === "needs_correction" ? "A few points need your help" : snapshot.generation.stage === "failed" || snapshot.generation.stage === "cancelled" ? "Creation needs attention" : "Bringing your pet to life"}</h2><p>{snapshot.generation.message || "Preparing your character…"}</p><div className="progress-track"><span style={{ width: `${snapshot.generation.progress}%` }} /></div><div className="progress-meta"><span>{snapshot.generation.stage.replaceAll("_", " ")}</span><span>{Math.round(snapshot.generation.progress)}%</span></div>{(snapshot.generation.stage === "failed" || snapshot.generation.stage === "cancelled") && snapshot.generation.error && <p className="generation-failure-detail" role="alert">{snapshot.generation.error}</p>}{snapshot.generation.stage === "completed" ? <div className="generation-recovery">{canRetrySavedRig && <button className="primary-button" disabled={busy} onClick={retryBlenderRig}><RefreshCcw /> Retry Blender rig</button>}{canRetrySavedAnimation && <button className="primary-button" disabled={busy} onClick={retryAnimationFlow}><RefreshCcw /> Retry animation</button>}<button className={canRetrySavedRig || canRetrySavedAnimation ? "ghost-button" : "primary-button"} onClick={() => setStep(3)}>Preview pet <ChevronRight /></button></div> : snapshot.generation.stage === "needs_correction" ? <div className="generation-recovery"><button className="primary-button" disabled={!snapshot.generation.candidateModelPath} onClick={beginRigCorrection}><Crosshair /> Help locate body parts</button><button className="ghost-button" disabled={busy} onClick={previewGeneratedModel}>Keep as a static 3D pet</button></div> : snapshot.generation.stage === "failed" || snapshot.generation.stage === "cancelled" ? <div className="generation-recovery">
          {snapshot.generation.rigAnalysis?.status === "corrected" && <button className="primary-button" disabled={busy} onClick={(snapshot.generation.error ?? "").toLowerCase().includes("animation") ? retryAnimationFlow : retryBlenderRig}>{busy ? <LoaderCircle className="spin" /> : <><RefreshCcw /> {(snapshot.generation.error ?? "").toLowerCase().includes("animation") ? "Retry animation only" : "Retry Blender rig"}</>}</button>}
          {snapshot.generation.taskId && !snapshot.generation.candidateModelPath && !files[0] && <button className="primary-button" disabled={busy} onClick={generate}>{busy ? <LoaderCircle className="spin" /> : <><RefreshCcw /> Recover saved 3D model</>}</button>}
          {snapshot.generation.candidateModelPath && <button className={snapshot.generation.rigAnalysis?.status === "corrected" ? "ghost-button" : "primary-button"} disabled={busy} onClick={previewGeneratedModel}>Preview generated 3D model <ChevronRight /></button>}
          {files[0] && <button className={snapshot.generation.candidateModelPath || snapshot.generation.rigAnalysis?.status === "corrected" ? "ghost-button" : "primary-button"} disabled={busy} onClick={generate}><RefreshCcw /> Try 3D again</button>}
          <button className="ghost-button" disabled={busy} onClick={continueWithImage}>Use image pet for now</button><button className="ghost-button" onClick={() => setStep(1)}><ChevronLeft /> Choose other images</button></div> : <button className="ghost-button center progress-action" onClick={cancelCreation}>Cancel creation</button>}</div>}
